...
extension CafeRepositoryProtocol {
  ...
  func add(name: String, visitDate: Date = Date(), comment: String?, rate: Int) -> CafeEntity {
    return add(name: name, visitDate: visitDate, comment: comment, rate: rate, latitude: nil, longitude: nil, address: nil, visitDates: [visitDate])
  }

  func update(id: String, name: String, visitDate: Date, comment: String?, rate: Int) {
    update(id: id, name: name, visitDate: visitDate, comment: comment, rate: rate, latitude: nil, longitude: nil, address: nil, visitDates: [visitDate])
  }
}
